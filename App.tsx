...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      const insight = await getWorkInsights();
  ...
  const handleSave = async () => {
    ...
    setEntries(updated);
    setIsSyncing(false);
    getWorkInsights().then(setAiInsight);
  };
  ...
  const handleNavigate = (months: number, years: number) => {
    const newDate = new Date(
      viewDate.getFullYear() + years,
      viewDate.getMonth() + months,
      1
    );
    setViewDate(newDate);
  };
      ...
      <div className="min-h-screen bg-indigo-600 dark:bg-slate-950 flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl p-8 shadow-2xl">
          ...
          >
            ...
            <button
              type="submit"
              className="w-full bg-indigo-600 text-white font-bold py-3 rounded-xl shadow-lg active:scale-95"
            >
              Login
            </button>
          </form>
        ...
        <div className="flex justify-between items-center relative z-10">
          <div>
            <p className="text-indigo-200 text-[10px] uppercase tracking-widest font-black flex items-center">
              Offline Workspace
              {isSyncing && (
                <span className="ml-2 w-1.5 h-1.5 bg-emerald-400 rounded-full animate-ping"></span>
              )}
            </p>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="bg-white/10 p-2.5 rounded-xl backdrop-blur-md"
            >
              {isDarkMode ? (
                <svg
          ...
          <main className="flex-1 p-6 space-y-6">
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800">
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center justify-between">
                ...
                {isSyncing && (
                  <span className="text-[10px] text-emerald-500 font-black animate-pulse">
                    SAVING...
                  </span>
                )}
              </h3>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl px-4 py-3.5 font-bold"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
              ...
              <div className="bg-indigo-50/50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50 rounded-2xl p-4 flex justify-between items-center mb-6">
                ...
                <div className="text-right">
                  ...
                  <p className="text-lg font-bold text-slate-700 dark:text-slate-300">
                    {(
                      totalMonthlyHours +
                      (entryMap[selectedDate.toISOString().split("T")[0]]
                        ? 0
                        : dailyDuration)
                    ).toFixed(1)}
                    h
              ...
              <button
                onClick={handleSave}
                disabled={isSyncing}
                className="w-full bg-slate-900 dark:bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-lg active:scale-95 uppercase tracking-widest text-sm disabled:opacity-50"
              >
            ...
            <div className="bg-gradient-to-br from-indigo-600 to-indigo-900 dark:from-indigo-900 dark:to-slate-900 rounded-3xl p-6 shadow-xl text-white relative overflow-hidden border border-indigo-500/20">
              ...
              <p className="text-sm font-semibold leading-relaxed italic relative z-10 text-indigo-50">
                "{aiInsight}"
              </p>
            </div>
          </main>
        ...
        <main className="flex-1 p-6 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center bg-white dark:bg-slate-900 p-1.5 rounded-xl border border-slate-100 dark:border-slate-800">
              <button onClick={() => handleNavigate(-1, 0)} className="p-1">
                &larr;
              </button>
              <span className="text-[10px] font-black uppercase mx-2 w-24 text-center">
          ...
          <div className="bg-emerald-500 dark:bg-emerald-600 p-6 rounded-3xl shadow-xl text-white">
            <p className="text-[10px] font-black text-emerald-100 uppercase tracking-[0.2em] mb-1">
              Projected Monthly Pay
            </p>
            <p className="text-4xl font-black">
              $
              {totalMonthlyEarnings.toLocaleString(undefined, {
            ...
            <div className="mt-4 pt-4 border-t border-white/20 flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
              ...
              <span>
                $
                {(totalMonthlyEarnings / (monthlyEntries.length || 1)).toFixed(
                  2
                )}
              </span>
            </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
                        {entry.startTime} &mdash; {entry.endTime}
                      </p>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">
                        {entry.duration.toFixed(1)} hrs &bull; $
                        {(entry.duration * hourlyWage).toFixed(2)}
                      </p>
                    </div>
                  </div>
          ...
          >
            ...
            >
              <path
                ...
                strokeLinejoin="round"
                strokeWidth="2.5"
                d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.407 2.67 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.407-2.67-1M12 17V7m0 10c-3.313 0-6-2.687-6-6s2.687-6 6-6 6 2.687 6 6-2.687 6-6 6z"
              />
            </svg>
            <span className="text-[10px] font-black uppercase tracking-widest">
              Earnings
